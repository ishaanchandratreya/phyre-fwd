"""
Documentation for the structure of the save directory

KL_Loss.html: Webpage showing the loss curve of the KL divergence inside of the world model
Train Rewards.html: Webpage showing the rewards that have been acquired at train time- not significant
Args.txt: Full list of arguments used for the running of the experiment
observation_loss.html: Webpage showing the reconstruction loss for the different terms

test_episode_{data_col_no}.mp4: Visualization of step-wise reconstruction at the stage without transition
test_episode_{data_col_no}.png: Visualization of step-wise reconstruction at the stage without transition (single image)

models_{data_col_no}.pth: Contains the stored model for the particular timestep
seed_info: contains log.txt explaining (task, action) pairs used for training and gifs of the action performed on the given task
data_col: contains folders at each episode explaining the data collected at that stage from the world model
...
from planner import MPCPlanner, LookAheadVisualizer
...
parser.add_argument('--stride', type=int, default=60, help='Given stride for forward prediction')
...
parser.add_argument('--t_viz', action='store_true', help='Visualize the learnt transition model in the latent space')
parser.add_argument('--rspace', action='store_true', help='Decides how to parametrize the action space (relative or not)')
...
metrics = {'steps': [], 'episodes': [], 'train_rewards': [], 'test_episodes': [], 'test_rewards': [],
           'observation_loss': [], 'kl_loss': []}

# Initialise training environment and experience replay memory
env = Env(args.env, args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth, int(args.stride))

default_env = Env(args.env, args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth, int(args.stride))
if args.experience_replay is not '' and os.path.exists(args.experience_replay):
    ...
    for s in range(1, args.seed_episodes + 1):
        ...
        #Add logging for different values of task, action (seed episode recording + videos)
        current_task, current_action = env.return_current_values()
        seed_tasks.append(current_task)
        seed_actions.append(current_action)
        info_act = None
        while not done:
            action = env.sample_random_action() if info_act is None else info_act
    ...
    save_current_tasks_and_actions_for_phyre(seed_tasks, seed_actions, os.path.join(results_dir, 'seed_info'), args)
...
# = RewardModel(args.belief_size, args.state_size, args.hidden_size, args.activation_function).to(
#    device=args.device)
...
def update_belief_and_act_without_reward(args, env, default_env, transition_model, encoder, belief, posterior_state, action,
    ...
    return belief, posterior_state, action, next_observation, reward, done, info, init_sample_action
...
if args.test:
    ...
    with torch.no_grad():
        ...
        for _ in tqdm(range(args.test_episodes)):
            ...
            info = None
            for t in pbar:
                belief, posterior_state, action, observation, reward, done, info, init_action = update_belief_and_act_without_reward(args,
                                                                                                                  env,
                                                                                                                  default_env,
                                                                                                                  transition_model,
                                                                                                                  encoder,
...
for episode in tqdm(range(metrics['episodes'][-1] + 1, args.episodes + 1), total=args.episodes,
    ...
    with torch.no_grad():
        ...
        current_task, _ = env.return_current_values()
        data_col_tasks.append(current_task)


        belief, posterior_state, action = torch.zeros(1, args.belief_size, device=args.device), torch.zeros(1,
                                                                                                            args.state_size,
                                                                                                            device=args.device), torch.zeros(1, env.action_size, device=args.device)
        pbar = tqdm(range(args.max_episode_length // args.action_repeat))
        info = None
        for t in pbar:
            belief, posterior_state, action, next_observation, reward, done, info, init_action = update_belief_and_act_without_reward(args,
                                                                                                                   env,
                                                                                                                   default_env,
                                                                                                                   transition_model,
                                                                                                                   encoder,
    ...
    save_current_tasks_and_actions_for_phyre(data_col_tasks, data_col_actions, os.path.join(results_dir, 'data_col', f'episode_{episode}'), args)
    # Test model
    #TODO: change back
    if episode % args.test_interval == 0:
        ...
        test_envs = PhyreEnvBatcher(Env, (
            args.env, args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth, int(args.stride)), {},
                               args.test_episodes)
        ...
        with torch.no_grad():
            ...
            current_tasks, _ = test_envs.return_current_values()
            ...
            for t in pbar:
                ...
                belief, posterior_state, action, next_observation, reward, done, info, init_action = update_belief_and_act_without_reward(
                    args, test_envs,
                    default_env,
                    transition_model,
                    encoder, belief,
                ...
                #This is where we check the initial action and record it
                if init_action is not None:
                    current_actions = [init_action] * len(current_tasks)
            ...
            save_current_tasks_and_actions_for_phyre(current_tasks, current_actions, os.path.join(results_dir, 'test_eps', f'episode_{episode}'), args)
