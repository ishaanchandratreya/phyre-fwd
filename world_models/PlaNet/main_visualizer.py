...
default_env = Env(args.env, args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth)
...
lookahead_use = LookAheadVisualizer(transition_model, observation_model, env.action_size, min_action=env.action_range[0], max_action=env.action_range[1], lookahead=12)
...
def update_belief_and_act_without_reward(args, env, transition_model, encoder, belief, posterior_state, action, observation, min_action=-inf, max_action=inf, explore=False):
  ...
  action = default_env.sample_random_action() if isinstance(env, EnvBatcher) else env.sample_random_action()  # Get action from planner(q(s_t|o≤t,a<t), p)
...
if args.test:
  ...
  with torch.no_grad():
    ...
    for _ in tqdm(range(args.test_episodes)):
      ...
      for t in pbar:
        belief, posterior_state, action, observation, reward, done = update_belief_and_act_without_reward(args, env, transition_model, encoder, belief, posterior_state, action, observation.to(device=args.device), env.action_range[0], env.action_range[1])
        total_reward += reward
        if args.render:
          env.render()
        if done:
...
for episode in tqdm(range(metrics['episodes'][-1] + 1, args.episodes + 1), total=args.episodes,
    ...
    with torch.no_grad():
        ...
        pbar = tqdm(range(args.max_episode_length // args.action_repeat))
        for t in pbar:
            belief, posterior_state, action, next_observation, reward, done = update_belief_and_act_without_reward(args, env,
                                                                                                    transition_model,
                                                                                                    encoder, belief,
                                                                                                    posterior_state,
                                                                                                    action,
    ...
    if episode % args.test_interval == 0:
        ...
        with torch.no_grad():
            ...
            for t in pbar:
                ...
                belief, posterior_state, action, next_observation, reward, done = update_belief_and_act_without_reward(args, test_envs,
                                                                                                        transition_model,
                                                                                                        encoder, belief,
                                                                                                        posterior_state,
                                                                                                        action,
