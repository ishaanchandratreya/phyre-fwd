...
def build_task(C, target_left, support_height, jar_scale, ball_scale):
    ...
    load = C.add('static bar', scale=0.6).set_bottom(support_height*C.scene.height)
    floor = C.add('static bar', scale=2).set_bottom(0)
    ...
    bounce = C.add('static bar', scale=0.2, right=jar_guard_l.left, restitution=1).set_bottom(floor.top)
    ...
    ball = C.add('dynamic ball', scale=ball_scale).set_bottom(load.top + 0.1*eps_h).set_left(0).set_vel_x(0.1*C.scene.width)
